...
describe('buildPlan — diff against completed + installed VERSION', () => {
  test('fresh install (no entries) — v0.11.0 is pending when installed ≥ 0.11.0', () => {
    ...
    // v0.12.0 (Knowledge Graph), v0.12.2 (JSONB repair), and v0.13.1
    // (Knowledge Runtime grandfather) are registered but installed VERSION is
    // 0.11.1, so they land in skippedFuture until the binary catches up.
    expect(plan.skippedFuture.map(m => m.version)).toEqual(['0.12.0', '0.12.2', '0.13.1']);
  });
  ...
  test('Codex H9 regression: installed > migration version → still runs (not skipped)', () => {
    ...
    expect(plan.pending.map(m => m.version)).toContain('0.11.0');
    // v0.12.2 and v0.13.1 were added later; installed=0.12.0 means they
    // belong in skippedFuture, not pending. v0.11.0 and v0.12.0 stay
    // pending despite being ≤ installed — that is the H9 invariant.
    expect(plan.skippedFuture.map(m => m.version)).toEqual(['0.12.2', '0.13.1']);
  });
