...
- **Action Brain commitment tracking shipped in this fork.** Added `src/action-brain/**` with schema init, extraction pipeline, action storage engine, brief generation, and MCP operations for listing, resolving, marking false positives, and ingesting commitments.
- **Action CLI surface for commitments.** Added `gbrain action list|brief|resolve|mark-fp|ingest` as user-facing aliases over Action Brain operations.
- **Daily action brief workflow.** Added ranked morning brief generation with urgency/confidence/recency scoring to surface the highest-priority open commitments.

### Fixed

- **Prompt-injection boundary hardening for commitment extraction.** Message batches are wrapped in structured delimiters before extraction so user text is treated as data.
- **Action operations connection handling.** Removed long-lived reserved Postgres connection usage in Action Brain query paths to avoid pool starvation under load.
